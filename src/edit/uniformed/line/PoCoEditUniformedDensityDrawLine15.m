...
// ============================================================================
@implementation PoCoEditUniformedDensityDrawLine15
...
{
    DPRINT((@"[PoCoEditUniformedDensityDrawLine15 init]\n"));
...
-(void)dealloc
{
    DPRINT((@"[PoCoEditUniformedDensityDrawLine15 dealloc]\n"));
      ...
      patternRow:(const int)prow
{
    ...
    DPRINT((@"[PoCoEditUniformedDensityDrawLine15 drawPoint:]\n"));
    ...
    bbmp += (brow - 6); pbmp += (prow - 6); [dp moveX:-6 moveY:1];
    if (([self->drawRect_ isPointInRect:dp]) &&
        (![[self->palette_ palette:*(bbmp)] isMask])) {
        *(bbmp) = [super calcColor:self->density_
                            color1:*(bbmp)
    ...
    for (i = 0; i < 4; i++) {
        bbmp += (brow - 14); pbmp += (prow - 14); [dp moveX:-14 moveY:1];
        if (([self->drawRect_ isPointInRect:dp]) &&
            (![[self->palette_ palette:*(bbmp)] isMask])) {
            *(bbmp) = [super calcColor:self->density_
                                color1:*(bbmp)
