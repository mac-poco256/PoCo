...
// ============================================================================
@implementation PoCoEditUniformedDensityDrawLine07
...
{
    DPRINT((@"[PoCoEditUniformedDensityDrawLine07 init]\n"));
...
-(void)dealloc
{
    DPRINT((@"[PoCoEditUniformedDensityDrawLine07 dealloc]\n"));
      ...
      patternRow:(const int)prow
{
    ...
    DPRINT((@"[PoCoEditUniformedDensityDrawLine07 drawPoint:]\n"));
    ...
    bbmp += (brow - 3); pbmp += (prow - 3); [dp moveX:-3 moveY:1];
    if (([self->drawRect_ isPointInRect:dp]) &&
        (![[self->palette_ palette:*(bbmp)] isMask])) {
        *(bbmp) = [super calcColor:self->density_
                            color1:*(bbmp)
