...
// ============================================================================
@implementation PoCoEditUniformedDensityDrawLine03
...
{
    DPRINT((@"[PoCoEditUniformedDensityDrawLine03 init]\n"));
...
-(void)dealloc
{
    DPRINT((@"[PoCoEditUniformedDensityDrawLine03 dealloc]\n"));
      ...
      patternRow:(const int)prow
{
    ...
    DPRINT((@"[PoCoEditUniformedDensityDrawLine03 drawPoint:]\n"));
    ...
    bbmp += (brow - 1); pbmp += (prow - 1); [dp moveX:-1 moveY:1];
    if (([self->drawRect_ isPointInRect:dp]) &&
        (![[self->palette_ palette:*(bbmp)] isMask])) {
        *(bbmp) = [super calcColor:self->density_
                            color1:*(bbmp)
