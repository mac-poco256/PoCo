...
// ============================================================================
@implementation PoCoEditUniformedDensityDrawLine16
...
{
    DPRINT((@"[PoCoEditUniformedDensityDrawLine16 init]\n"));
...
-(void)dealloc
{
    DPRINT((@"[PoCoEditUniformedDensityDrawLine16 dealloc]\n"));
      ...
      patternRow:(const int)prow
{
    ...
    DPRINT((@"[PoCoEditUniformedDensityDrawLine16 drawPoint:]\n"));
    ...
    for (i = 0; i < 5; i++) {
        bbmp += (brow - 15); pbmp += (prow - 15); [dp moveX:-15 moveY:1];
        if (([self->drawRect_ isPointInRect:dp]) &&
            (![[self->palette_ palette:*(bbmp)] isMask])) {
            *(bbmp) = [super calcColor:self->density_
                                color1:*(bbmp)
