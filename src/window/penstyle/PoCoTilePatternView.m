...
-(void)setSelection:(unsigned int)num
{
    ...
    if (num != sel) {
        [[(PoCoAppController *)([NSApp delegate]) editInfo] setTileNumber:num];
...
{
    DPRINT((@"[PoCoTilePatternView initWithFrame]\n"));

    // super class の初期化
    self = [super initWithFrame:frame
                     setPattern:[(PoCoAppController *)([NSApp delegate]) tilePattern]];
...
-(void)mouseDown:(NSEvent *)evt
{
    ...
    if ([evt clickCount] >= 2) {
        ...
        [wnd raisePatternSheet:self
                       pattern:[[(PoCoAppController *)([NSApp delegate]) tilePattern] pattern:[[(PoCoAppController *)([NSApp delegate]) editInfo] tileNumber]]];
    }
...
-(void)nextSelection
{
    unsigned int num = [[(PoCoAppController *)([NSApp delegate]) editInfo] tileNumber];
