...
// ============================================================================
@implementation PoCoPaletteGradationView
...
//    function : 選択番号
//
-(int)selColor:(int)num event:(NSEvent *)evt
{
...
{
    DPRINT((@"[PoCoPaletteGradationView initWithFrame]\n"));
...
-(void)dealloc
{
    DPRINT((@"[PoCoPaletteGradationView dealloc]\n"));
...
-(void)drawRect:(NSRect)rect
{
    ...
    DPRINT((@"[PoCoPaletteGradationView drawRect]\n"));

    for (l = 0; l < COLOR_MAX; (l)++) {
        if (NSIntersectsRect(rect, [self numToRect:l])) {
            [self drawColor:l
                   isSelect:((self->leftNum_ == l) || (self->rightNum_ == l))
