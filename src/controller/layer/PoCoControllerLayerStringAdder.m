...
// ============================================================================
@implementation PoCoControllerLayerStringAdder
...
{
    DPRINT((@"[PoCoControllerLayerStringAdder init]\n"));
...
-(void)dealloc
{
    DPRINT((@"[PoCoControllerLayerStringAdder dealloc]\n"));
...
-(BOOL)execute
{
    ...
    // 常に最上位に追加する
    er = [self->picture_ addStringLayer:-1 color:self->color_];
    if (er < ER_OK) {
...
-(void)createUndo
{
    ...
    [super setUndoName:[[NSBundle mainBundle]
                           localizedStringForKey:@"LayerCreateString"
                                           value:@"create string layer"
                                           table:nil]]; 
