...
// ============================================================================
@implementation PoCoControllerPaletteDecrementer
...
-(id)init:(PoCoPicture *)pict
   ...
   buffer:(PoCoColorBuffer *)buf
    ...
    green:(BOOL)g
     ...
     step:(unsigned int)s
{
//    DPRINT((@"[PoCoControllerPaletteDecrementer init]\n"));
...
-(void)dealloc
{
//    DPRINT((@"[PoCoControllerPaletteDecrementer dealloc]\n"));
...
//    DPRINT((@"[PoCoControllerPaletteDecrementer execute]\n"));
    ...
    if (self->isRed_) {
        elm[0] = MAX(0, elm[0] - (int)(self->step_));
    }
    elm[1] = (int)([col green]);
    self->prevElm_[1] = elm[1];
    if (self->isGreen_) {
        elm[1] = MAX(0, elm[1] - (int)(self->step_));
    }
    elm[2] = (int)([col blue]);
    self->prevElm_[2] = elm[2];
    if (self->isBlue_) {
        elm[2] = MAX(0, elm[2] - (int)(self->step_));
    }
        ...
        createPaletteSingleSetter:YES
                          ...
                          isTrans:[[[self->picture_ palette] palette:self->num_] isTrans]
                             name:[[NSBundle mainBundle]
                                      localizedStringForKey:@"PaletteDecrement"
                                                      value:@"decrement palette"
                                                      table:nil]];
